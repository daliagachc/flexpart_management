...
def plot_table_ecoregions(gdf18, mega1, figsize=(7.5, 4.5), min_per=5, tw=40):
    import textwrap
    my_wrap = textwrap.TextWrapper(width=tw)
    _m = mega1[['inf_index', 'name', 'geometry', 'typ']]
    _m['nname'] = _m.apply(
        lambda r: my_wrap.fill(f'{r["inf_index"]:2d}.{r["name"]}-{r["typ"]}'),
        axis=1
    )
...
def get_extra_gdf(lake_pol, pacific_ocean_pol):
    ex = {
        0: {
            ...
            'geometry': lake_pol,
            'color'   : 'lavender',
            'parent'  : 'Montane Grasslands & Shrublands'
        },
        1: {
...
def get_ecoregion_gdf(ecoregions_trimmed):
    e1 = ecoregions_trimmed[
        ['ECO_NAME', 'ECO_ID', 'geometry', 'COLOR', 'BIOME_NAME']
    ]
    e1 = e1.rename({
        'ECO_ID'  : 'code',
        'ECO_NAME': 'name',
...
def get_ecoregion_gdf_biome(ecoregions_trimmed):
    e1 = ecoregions_trimmed[
        ['ECO_NAME', 'ECO_ID', 'geometry', 'COLOR',
         'BIOME_NAME', 'COLOR_BIO']
    ]
    e1 = e1.rename({
        'ECO_ID'  : 'code',
        'BIOME_NAME': 'name',
...
def get_mer_gdf(meow, all_eco):
    ...
    # %%
    m1 = meow[['ECO_CODE', 'ECOREGION',
               'geometry','PROVINCE',
               'REALM'
               ]]
    m1 = m1.rename({'ECO_CODE': 'code', 'ECOREGION': 'name',
                   'PROVINCE':'parent'}, axis=1)
    m1['typ'] = 'MER'
